...
/**
 * Bir oyuncunun sıralamadaki yerine göre hangi piramit kademesinde olduğunu
 * (ve o kademenin güç puanını) hesaplar. rank=1 en üst, rank=totalPlayers en alt.
 */
export function tierPowerForRank(rank, totalPlayers) {
  if (!totalPlayers || totalPlayers <= 0) return BOTTOM_TIER_POWER;
  const tierCount = PYRAMID_TIERS.length;
  // Pozisyon: 1 = en alt, totalPlayers = en üst.
  const positionFromBottom = totalPlayers - rank + 1;
  const bucketIndex = Math.min(
    tierCount - 1,
    Math.floor(((positionFromBottom - 1) * tierCount) / totalPlayers)
  );
  return PYRAMID_TIERS[bucketIndex];
}
